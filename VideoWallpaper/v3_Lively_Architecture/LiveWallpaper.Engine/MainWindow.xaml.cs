...
public partial class MainWindow : Window
{
    ...
    private bool _isScreensaverMode = false;
    private System.Windows.Point _lastScreensaverMousePos;
    public int Volume
    {
    ...
    public MainWindow(Screen? targetScreen = null, bool isScreensaver = false)
    {
        InitializeComponent();
        _targetScreen = targetScreen ?? Screen.PrimaryScreen ?? throw new Exception("No primary screen detected.");
        _isScreensaverMode = isScreensaver;
    ...
    private void Window_SourceInitialized(object? sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                this.MouseDown += Screensaver_KeyDown;
            }
        }
    ...
    private void Screensaver_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
    {
        System.Windows.Point pos = e.GetPosition(this);
        if (_lastScreensaverMousePos == default) 
        {
            _lastScreensaverMousePos = pos;
        }
        else if (Math.Abs(_lastScreensaverMousePos.X - pos.X) > 5 || Math.Abs(_lastScreensaverMousePos.Y - pos.Y) > 5)
        {
            System.Windows.Application.Current.Shutdown();
        }
    }

    private void Screensaver_KeyDown(object sender, EventArgs e)
    {
        System.Windows.Application.Current.Shutdown();
    }
}
